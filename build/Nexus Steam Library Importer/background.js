...
async function scrapeSteamLibrary(profileInput) {
  ...
  try {
    tab = await chrome.tabs.create({ active: false, url: normalized.libraryUrl });
...
async function handleScan(message) {
  const [steam, catalog] = await Promise.all([
    scrapeSteamLibrary(message.profileInput),
  ...
  const results = matchLibrary(steam.games, catalog, currentGames);

  await setStored(SETTINGS_KEY, {
    lastProfileUrl: steam.profileUrl,
    updatedAt: Date.now()
  });

  return {
    profileName: steam.profileName,
    profileUrl: steam.profileUrl,
    steamGameCount: steam.games.length,
    expectedSteamGameCount: steam.expectedCount,
...
async function notifyJobOwner(job) {
  ...
  try {
    await chrome.tabs.sendMessage(job.ownerTabId, {
      type: "NSI_JOB_UPDATE",
      job: publicJob(job)
    });
  } catch {
...
function validateJobItems(items) {
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error("Select at least one matched game.");
  }
  if (items.length > 500) {
    throw new Error("A single import is limited to 500 games.");
  }

  return items.map((item) => {
    const domain = String(item.domain || "").toLowerCase().trim();
    const nexusName = String(item.nexusName || "").trim();
    const steamName = String(item.steamName || "").trim();
    if (!/^[a-z0-9][a-z0-9._;'-]{0,120}$/.test(domain) || !nexusName || nexusName.length > 200) {
      throw new Error("One of the selected Nexus matches is invalid.");
    }
    return {
      steamAppId: String(item.steamAppId || ""),
      steamName: steamName.slice(0, 200),
      nexusName,
      domain,
      status: "queued",
      error: null
...
async function startImport(items, ownerTabId) {
...
async function processWorkerTab(job, tab) {
  ...
  try {
    const response = await sendMessageWithRetry(tab.id, {
      type: "NSI_BOOKMARK_NEXUS_GAME",
      game: { name: item.nexusName, domain: item.domain }
    });
    if (!response?.ok) {
      throw new Error(response?.error || "Nexus did not confirm the bookmark.");
    }
    ...
    latestItem.status = response.status === "already" ? "already" : "added";
...
async function pumpJob() {
  ...
  try {
    ...
    if (Number.isInteger(job.workerTabId)) {
      try {
        const workerTab = await chrome.tabs.get(job.workerTabId);
...
chrome.action.onClicked.addListener(async () => {
  const tabs = await chrome.tabs.query({ url: "https://www.nexusmods.com/" });
  let tab = tabs[0];
  if (tab?.id) {
    await chrome.tabs.update(tab.id, { active: true });
  } else {
    tab = await chrome.tabs.create({ active: true, url: "https://www.nexusmods.com/" });
    await waitForTabComplete(tab.id);
  }
...
chrome.runtime.onStartup.addListener(() => schedulePump(500));
schedulePump(500);
