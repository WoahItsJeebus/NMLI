(function initMatcher(root, factory) {
  ...
  function findBestMatch(sourceGame, catalog, catalogByDomain, preparedCatalog) {
    const providerId = String(sourceGame.providerId || "steam").toLowerCase();
    const aliasDomain = providerId === "steam"
      ? NEXUS_ALIASES_BY_STEAM_APP[String(sourceGame.appId || "")]
      : null;
    ...
    let best = null;
    let secondScore = 0;
    const steamPrepared = prepareName(sourceGame.name);
  ...
  function matchLibrary(sourceGames, rawCatalog, currentGames = []) {
    ...
    for (const sourceGame of sourceGames || []) {
      const cleaned = {
        appId: String(sourceGame.appId || ""),
        name: String(sourceGame.name || "").trim(),
        providerId: String(sourceGame.providerId || "steam").toLowerCase()
      };

      if (!cleaned.appId || !cleaned.name || isLikelyNonGame(cleaned)) {
        results.push({ source: cleaned, steam: cleaned, status: "not_found", skipped: true, match: null });
        continue;
      }

      const match = findBestMatch(cleaned, catalog, catalogByDomain, preparedCatalog);
      if (!match) {
        results.push({ source: cleaned, steam: cleaned, status: "not_found", match: null });
        continue;
      }
    ...
    return results.sort((left, right) => {
      ...
      return order[left.status] - order[right.status] || left.source.name.localeCompare(right.source.name);
    });
  }
